...
justprompt --list          # see what tasks are available
justprompt --dry-run test  # preview without calling the LLM
justprompt test            # run it for real
```
...
Tasks can depend on other tasks, accept arguments, and use different LLM providers. Run `justprompt --list` in any example directory to see what's available.
